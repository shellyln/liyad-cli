...
function getInterpreter(profileName, lsxBootPath) {
    switch (profileName) {
    case 'S':
        return liyad.S;
    case 'L': case 'lisp':
    ...
    case 'LSX': case 'LSX_async':
        {
            const lsxBoot = require(lsxBootPath);
            return liyad.LSX({
                jsx: lsxBoot.dom,
    ...
    default:
        throw new Error(`Unknown profile name is specified: ${profileName} .`);
    }
}
...
function cli() {
    ...
    let lsxBootPath = '../examples/lsxboot';
    let evalCode = void 0;
    let optsPos = 2;
